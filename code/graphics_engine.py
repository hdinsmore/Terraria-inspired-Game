...
class Camera:
    def __init__(self, center: pg.Vector2): 
        ...
        self.max_x = (MAP_SIZE[0] * TILE_SIZE) - (RES[0] // 2)
        self.max_y = (MAP_SIZE[1] * TILE_SIZE) - (RES[1] // 2)
...
class TerrainGraphics:
    def __init__(self, game_obj: Main):
        ...
        self.chunk_manager: ChunkManager = game_obj.chunk_manager
        self.tile_map = game_obj.proc_gen.tile_map
        self.names_to_ids: dict[str, int] = game_obj.proc_gen.names_to_ids
        self.ids_to_names: dict[int, str] = game_obj.proc_gen.ids_to_names
        ...
        self.elevation_data = self.get_elevation_data(self.current_biome)
    ...
    def get_elevation_data(self, current_biome: str) -> dict[str, int]:
        elev_params = BIOMES[current_biome]['elevation']
    ...
    def render_bg_imgs(self, bg_type: str) -> None:
        base_y = self.elevation_data['landscape base']
        imgs = list(self.asset_manager.graphics['backgrounds'][self.current_biome][bg_type].values())
        num_layers = len(imgs)
        x_offset = self.biome_x_offsets[self.current_biome]
        for i in range(num_layers):
            ...
            if bg_type == 'landscape':
                ...
                start_x = floor((scroll_x - x_offset) / img_width)
                for x in range(start_x, start_x + num_imgs_x):
                    self.screen.blit(img, ((x_offset + x * img_width) - scroll_x, base_y - layer_offset_y - self.cam_offset.y))  
            else: # underground
                start_x = floor((self.cam_offset.x - x_offset) / img_width)
                dist_y = self.elevation_data['underground span']
                layer_dist_y = ceil(dist_y / num_layers)
                num_imgs_y = ceil(layer_dist_y / img_height)
                for x in range(start_x, start_x + num_imgs_x):
                    for y in range(num_imgs_y):
                        self.screen.blit(img, (x_offset + (img_width * x), base_y + (img_height * y)) - self.cam_offset)
    
    # in a separate method from render_tiles to render in front of the player
    def render_water(self) -> None:
        for chunk in self.chunk_manager.visible_chunks:
            for (x, y) in chunk:
                if self.tile_map[x, y] == self.names_to_ids['water']:
                    self.screen.blit(
                        self.asset_manager.get_image('water'), 
                        pg.Vector2(x * TILE_SIZE, y * TILE_SIZE) - self.cam_offset
                    )

    def get_mined_tile_img(self, x: int, y: int) -> None:
        '''reduce the opacity of a given tile as it's mined away'''
        tile_image = self.asset_manager.get_image(self.get_tile_type(x, y)).copy()
    ...
    def update(self, current_biome: str) -> None:
        ...
        self.chunk_manager.render_chunks()
...
class BiomeTransition:
    ...
    def init_alphas(self, current_biome: str) -> None:
        for bg_type in self.bg_types:
            for img in self.asset_manager.graphics['backgrounds'][self.previous_biome][bg_type].values():
                img.set_alpha(255) 

            for img in self.asset_manager.graphics['backgrounds'][current_biome][bg_type].values():
    ...
    def run(self, current_biome: str) -> None:
        ...
        for bg_type in self.bg_types:
            previous_biome_imgs = list(self.asset_manager.graphics['backgrounds'][self.previous_biome][bg_type].values())
            current_biome_imgs = list(self.asset_manager.graphics['backgrounds'][current_biome][bg_type].values())
